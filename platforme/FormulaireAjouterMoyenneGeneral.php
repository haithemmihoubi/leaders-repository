             ...
             ?> 
                      ...
                      <h5>Moyenne Générale</h5>
                        ...
                        <form class="user" action="#" method="post"  enctype="multipart/form-data">
                                <input type="number" id="moyenneGeneral" name="moyenneGeneral" id="moyenneGeneral"  value="<?php echo sprintf("%.02f",($moy/$AllCoef)) ?>">
                            

                            <button  type="submit" class="btn btn-primary btn-user  btn-block" name="submit">Enregistrer dans la  base de données</button>
...
{
    $moyenneGeneral=htmlspecialchars($_POST['moyenneGeneral']);
    

    $verife=$cnx->query("SELECT  cin_etudiant FROM bulletin WHERE cin_etudiant='$cin_etudiant'");
    if($verife->rowCount()>0) 
    {
        $sql2="UPDATE bulletin set  moyenneGeneral='$moyenneGeneral' WHERE cin_etudiant='$cin_etudiant'";   
        $res=$cnx->exec($sql2);
    }
    else
    {
        $sql="INSERT INTO bulletin (cin_etudiant,moyenneGeneral) VALUES ('$cin_etudiant','$moyenneGeneral')";
        $res=$cnx->exec($sql);
	...
	if($res)
    {
       ?>
       <script>
		    swal("Moyenne!", "a été Ajouter!", "success")
</script>
<?php
    }
?>
